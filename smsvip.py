...
for i in range(count):
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        time.sleep(2)
        executor.submit(fptreg)
        executor.submit(fptreset)
        executor.submit(fptresend)
        ...
        executor.submit(vulcano)
        executor.submit(vulcanoreset)
        executor.submit(frontrow)
        executor.submit(bibomart)
        executor.submit(sbiz)
        executor.submit(hachihachi)
        executor.submit(hachihachiresend)
